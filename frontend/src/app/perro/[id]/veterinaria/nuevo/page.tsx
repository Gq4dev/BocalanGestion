...
export default function NuevoVeterinariaPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      await api.veterinaryRecords.create({
        dog: dogId,
        date: form.date,
        type: form.type,
  ...
  return (
    <div className="max-w-xl mx-auto space-y-6">
      <div className="flex items-center gap-4">
        ...
        <h1 className="text-2xl font-bold">Nuevo registro veterinario</h1>
      </div>
      <form onSubmit={handleSubmit} className="card space-y-4">
        ...
        <div>
          ...
          <input className="input" value={form.description} onChange={(e) => setForm((f) => ({ ...f, description: e.target.value }))} placeholder="Ej. Vacuna antirrábica anual" />
        </div>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div>
